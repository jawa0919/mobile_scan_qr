...
import com.example.mobile_scan_qr.objects.BarcodeFormats
import com.example.mobile_scan_qr.objects.DetectionSpeed
...
class MobileScannerHandler(
    ...
    private val analyzeImageSuccessCallback: AnalyzerSuccessCallback = {
        Handler(Looper.getMainLooper()).post {
            analyzerResult?.success(mapOf(
                "name" to "barcode",
                "com.example.mobile_scan_qr.getData" to it
            ))
            analyzerResult = null
        }
    }
    ...
    private val callback: MobileScannerCallback = { barcodes: List<Map<String, Any?>>, image: ByteArray?, width: Int?, height: Int? ->
        if (image != null) {
            barcodeHandler.publishEvent(mapOf(
                "name" to "barcode",
                "com.example.mobile_scan_qr.getData" to barcodes,
                "image" to image,
    ...
    private val errorCallback: MobileScannerErrorCallback = {error: String ->
        barcodeHandler.publishEvent(mapOf(
            "name" to "error",
            "com.example.mobile_scan_qr.getData" to error,
        ))
    }
    ...
    private val torchStateCallback: TorchStateCallback = {state: Int ->
        barcodeHandler.publishEvent(mapOf("name" to "torchState", "com.example.mobile_scan_qr.getData" to state))
    }

    private val zoomScaleStateCallback: ZoomScaleStateCallback = {zoomScale: Double ->
        barcodeHandler.publishEvent(mapOf("name" to "zoomScaleState", "com.example.mobile_scan_qr.getData" to zoomScale))
    }

    init {
        methodChannel = MethodChannel(binaryMessenger,
            "com.example.mobile_scan_qr/methodChannel")
