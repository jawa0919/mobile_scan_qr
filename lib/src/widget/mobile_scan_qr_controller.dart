...
import '../util/mobile_scan_qr_error_code.dart';
import '../enums/torch_state.dart';
import '../util/mobile_scan_qr_exception.dart';
import '../util/mobile_scan_qr_view_attributes.dart';
import '../objects/barcode_capture.dart';
import '../util/mobile_scan_qr_state.dart';
import '../objects/start_options.dart';

/// The controller for the [MobileScanner] widget.
class MobileScanQrController extends ValueNotifier<MobileScanQrState> {
  /// Construct a new [MobileScanQrController] instance.
  MobileScanQrController({
    ...
    this.useNewCameraSelector = false,
        ...
        super(MobileScanQrState.uninitialized(facing));
  ...
  void _setupListeners() {
    _barcodesSubscription = MobileScanQrPlatform.instance.barcodesStream
    ...
    _torchStateSubscription = MobileScanQrPlatform.instance.torchStateStream
    ...
    _zoomScaleSubscription = MobileScanQrPlatform.instance.zoomScaleStateStream
  ...
  void _throwIfNotInitialized() {
    if (!value.isInitialized) {
      throw const MobileScanQrException(
        errorCode: MobileScanQrErrorCode.controllerUninitialized,
        errorDetails: MobileScannerErrorDetails(
          message: 'The MobileScanQrController has not been initialized.',
        ),
      );
    }
    ...
    if (_isDisposed) {
      throw const MobileScanQrException(
        errorCode: MobileScanQrErrorCode.controllerDisposed,
        errorDetails: MobileScannerErrorDetails(
          message:
              'The MobileScanQrController was used after it has been disposed.',
        ),
      );
    }
  }
  ...
  Future<BarcodeCapture?> analyzeImage(String path) {
    return MobileScanQrPlatform.instance.analyzeImage(path);
  }
  ...
  Widget buildCameraView() {
    ...
    return MobileScanQrPlatform.instance.buildCameraView();
  }
  ...
  Future<void> resetZoomScale() async {
    ...
    await MobileScanQrPlatform.instance.resetZoomScale();
  }
  ...
  Future<void> setZoomScale(double zoomScale) async {
    ...
    // When the platform has updated the zoom scale,
    // it will send an update through the zoom scale state event stream.
    await MobileScanQrPlatform.instance.setZoomScale(clampedZoomScale);
  }
  ...
  Future<void> start({CameraFacing? cameraDirection}) async {
    ...
    try {
      ...
      final MobileScanQrViewAttributes viewAttributes =
          await MobileScanQrPlatform.instance.start(
        options,
      );
    ...
    } on MobileScanQrException catch (error) {
  ...
  Future<void> stop() async {
    ...
    await MobileScanQrPlatform.instance.stop();
  }
  ...
  Future<void> toggleTorch() async {
    ...
    await MobileScanQrPlatform.instance.setTorchState(newState);
  }
  ...
  Future<void> updateScanWindow(Rect? window) async {
    ...
    await MobileScanQrPlatform.instance.updateScanWindow(window);
  }
  ...
  Future<void> dispose() async {
    ...
    await MobileScanQrPlatform.instance.dispose();
  }
}
